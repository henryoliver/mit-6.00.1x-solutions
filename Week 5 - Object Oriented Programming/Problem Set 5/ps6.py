...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        bestShift = { 'words': 0, 'value': 0, 'message': '' }

        for n in range(26):
            decrypted = self.apply_shift(n)
            words = decrypted.split()

            validWords = list(word for word in words if is_word(self.valid_words, word))
            validWordsLen = len(validWords)

            if (validWordsLen > bestShift['words']):
                bestShift.update({ 'words': validWordsLen, 'value': n, 'message': decrypted })

        return (bestShift['value'], bestShift['message'])

def decrypt_story():
    cipher_text = CiphertextMessage(get_story_string())
    decr = cipher_text.decrypt_message()
